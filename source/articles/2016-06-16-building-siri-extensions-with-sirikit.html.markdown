---
title: Building Siri Extensions with SiriKit
date: 2016-06-16 14:12 UTC
tags: swift, siri
---

With the introduction of iOS 10 at WWDC 2016, Siri has finally been opened up to developers, in the form of [SiriKit](https://developer.apple.com/sirikit/).

SiriKit supports a fixed set of app categories for now, ranging from messaging and phone calls to payments and booking rides. Siri is an extremely complicated product, having to manage many languages and process words correctly, so I wanted to see how easy it is to add Siri support to an app.

I’m going to walk through the entire process of building a basic payments app, covering domains, intents, and the overall SiriKit integration itself.
...
Before getting started, I want to go over how the app will be structured. The only two concepts I’m interested in are contacts, the people to whom we will be sending payments, and the payments themselves. For now, payments values will just be integers, so we don’t have to worry about currencies.

SiriKit integrates nicely with Contacts to retrieve the names of people we want to send contacts to, but just to mix it up, I’m not going to use that – instead, the app will provide its own contacts through `INPerson`.
...
If you’d like to follow along, create a new project in Xcode 8 using the single view controller template. This is all you need for now – be sure that it builds on device though, as Siri does not have simulator support.
...
Note that, because a framework is used here, you have to concern yourself with access control. The class and its useful functions must be declared as `public`.
